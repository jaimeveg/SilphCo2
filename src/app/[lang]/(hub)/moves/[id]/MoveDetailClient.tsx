...
import { Swords, Sparkles, Shield, Flame, Activity, Target, Zap, AlertTriangle, ArrowLeft, TrendingUp, TrendingDown, User, Crosshair, GripHorizontal } from 'lucide-react';
    ...
    return (
        <div className="max-w-6xl mx-auto p-3 md:p-6 space-y-4 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="flex items-center gap-6 mb-2">
                <button onClick={() => router.back()} className="flex items-center gap-2 text-slate-400 hover:text-cyan-400 text-xs font-mono uppercase tracking-widest transition-colors w-fit group">
                    <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" />
                    {lang === 'es' ? 'VOLVER' : 'BACK'}
                </button>
                <Link href={`/${lang}/moves`} className="flex items-center gap-2 text-slate-500 hover:text-purple-400 text-xs font-mono uppercase tracking-widest transition-colors w-fit">
                    <GripHorizontal size={14} />
                    {lang === 'es' ? 'VER TODOS' : 'VIEW ALL MOVES'}
                </Link>
            </div>
            ...
            <div className="bg-slate-900/80 backdrop-blur-md border border-slate-700/50 rounded-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.5)] relative overflow-hidden">
                ...
                {/* CORRECCIÓN DE LAYOUT: Restablecido a md:flex-row para que ocupe la derecha (el recuadro verde) */}
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10 border-b border-slate-800 pb-4 mb-4">
                    <div className="flex flex-col gap-1.5 w-full md:w-auto">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            {move.is_sheer_force_boosted && (
                                <span className="bg-slate-950 border border-slate-800 text-slate-400 px-2.5 py-0.5 rounded text-[10px] font-mono font-bold uppercase tracking-widest flex items-center gap-1.5" title="Beneficiado por la habilidad Sheer Force">
                    ...
                    {/* CORRECCIÓN: Añadido shrink-0 y overflow-hidden para mantener el formato compacto */}
                    <div className="flex bg-slate-950 border border-slate-700 rounded-xl overflow-hidden shadow-inner w-full md:w-auto mt-2 md:mt-0 shrink-0">
                        <div className="flex flex-col items-center px-3 py-2 hover:bg-slate-900 transition-colors">
                        ...
                        <div className="flex flex-col items-center px-3 py-2 hover:bg-slate-900 transition-colors">
                            <span className="text-[9px] text-cyan-500 font-mono font-bold tracking-widest mb-0.5">BASE PP</span>
                            <span className="text-xl font-black text-white font-display leading-none">{move.pp || '-'}</span>
                        </div>
                        <div className="w-px bg-slate-800 shrink-0" />

                        <div className="flex flex-col items-center px-3 py-2 hover:bg-slate-900 transition-colors bg-slate-900/30">
                            <span className="text-[9px] text-amber-500 font-mono font-bold tracking-widest mb-0.5">MAX PP</span>
                            <span className="text-xl font-black text-amber-400 font-display leading-none">{move.max_pp || '-'}</span>
                        </div>
                    </div>
                </div>
