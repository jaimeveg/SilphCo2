...
import { TYPES, calculateDefense } from '@/lib/typeLogic'; // Asume que creaste el archivo arriba
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 p-6 md:p-12">
      ...
      <header className="max-w-6xl mx-auto flex items-center justify-between mb-12">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-slate-400 hover:text-brand-cyan transition-colors px-4 py-2 border border-transparent hover:border-brand-cyan/30 rounded-lg"
        >
          <ArrowLeft size={20} />
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 shadow-xl backdrop-blur-sm">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-sm font-mono text-slate-400 uppercase tracking-widest">
                Selecciona Tipos ({selectedTypes.length}/2)
              </h2>
              {selectedTypes.length > 0 && (
                <button
              ...
              {TYPES.map((type) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`opacity-${
                      isDisabled ? '40' : '100'
                    } grayscale-${isDisabled ? '100' : '0'}`}
                    disabled={isDisabled}
                  >
                    <div
                      className={`transform transition-all ${
                        isSelected
                          ? 'scale-110 ring-2 ring-brand-cyan rounded-full'
                          : ''
                      }`}
                    >
          ...
          {selectedTypes.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center opacity-30">
              ...
              <p className="font-mono text-sm">
                SELECCIONA TIPOS PARA INICIAR SIMULACIÓN
              </p>
            </div>
          ) : (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              {/* Bloque: Debilidades Extremas (x4) */}
              {grouped.x4.length > 0 && (
                <ResultRow
                  title="AMENAZA CRÍTICA (x4)"
                  types={grouped.x4}
                  color="bg-red-500"
                />
              )}

              {/* Bloque: Debilidades (x2) */}
              {grouped.x2.length > 0 && (
                <ResultRow
                  title="DEBILIDADES (x2)"
                  types={grouped.x2}
                  color="bg-orange-500"
                />
              )}
