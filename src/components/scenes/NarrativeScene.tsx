...
const WeatherSim = dynamic(() => import('./visuals/simulations/WeatherSim'), {
  loading: () => <div className="h-full flex items-center justify-center text-cyan-500 animate-pulse">LOADING...</div>,
});
...
export default function NarrativeScene({
  ...
  const pendingNavigation = useRef<{ originY: number; targetY: number } | null>(null);
  ...
  const renderVisualModule = () => {
    ...
    switch (legacyConfig.type) {
      ...
      case 'dual':
        // AQUI PASAMOS LANG
        return <VisualDual pokeId={legacyConfig.pokeId} dict={dict} lang={lang} />;
      case 'concept':
        return (
          <VisualConcept
            pokeId={legacyConfig.pokeId}
  ...
  const renderActionButtons = () => {
      ...
      return (
        <div className="mt-8 pt-6 border-t border-slate-800/50">
          <button
            ...
            className="inline-flex items-center gap-2 text-xs font-mono text-cyan-500 hover:text-white transition-colors group cursor-pointer focus:outline-none"
          >
            <span className="border-b border-cyan-500/30 group-hover:border-cyan-500 pb-0.5 uppercase">
              {t.related_category}
            </span>
            <ArrowRight
              size={12}
    ...
    if (data.id.includes('move') || data.id.includes('stab')) {
      return (
        <div className="mt-8 pt-6 border-t border-slate-800/50">
          <button className="flex items-center gap-2 px-4 py-2 bg-slate-900 border border-slate-700 rounded-md text-xs font-mono text-slate-400 cursor-not-allowed opacity-70 hover:opacity-100 transition-opacity uppercase">
            <Database size={12} />
            <span>{t.db_wip}</span>
          </button>
        </div>
      ...
      >
        <div className="relative aspect-square lg:aspect-[4/3] rounded-3xl bg-slate-900/40 border border-white/10 backdrop-blur-sm overflow-hidden flex items-center justify-center shadow-2xl group">
          ...
          <div className="absolute top-6 right-6 bg-slate-950/80 backdrop-blur-md px-4 py-2 rounded border border-white/10 z-30 pointer-events-none">
            <span className="block text-[10px] text-cyan-500 font-mono tracking-widest uppercase mb-1">
              {t.data_point}
            </span>
            <TextScramble
        ...
        <div className="relative z-20 space-y-10 pl-4">
          ...
          <div className="grid grid-cols-1 gap-4 pt-8 mt-4">
            <span className="text-xs font-mono text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-2">
              <span className="w-1 h-1 bg-slate-500 rounded-full"></span> {common.system_notes}
            </span>
            ...
            {data.content.technical_notes?.map((note, i) => (
              <motion.div
                ...
                className="group relative overflow-hidden bg-slate-900 border-l-2 border-slate-700 hover:border-cyan-500 p-5 transition-all duration-300 rounded-r-lg"
              >
                <div className="absolute inset-0 bg-cyan-500/5 translate-x-full group-hover:translate-x-0 transition-transform duration-500 ease-out" />
                <div className="relative z-10">
                  <div className="flex items-baseline gap-3 mb-2">
                    <span className="text-cyan-500 font-bold font-mono text-xs uppercase tracking-wider">
                      [{note.label}]
                    </span>
                    <div className="h-px flex-1 bg-slate-800 group-hover:bg-cyan-500/30 transition-colors" />
                  </div>
      ...
      <AnimatePresence>
        {navState?.isActive && (
          <motion.button
            ...
            className="fixed bottom-12 right-12 z-50 flex items-center gap-3 px-5 py-3 bg-cyan-500 text-slate-950 rounded-full shadow-[0_0_30px_rgba(56,189,248,0.4)] font-bold font-mono text-xs hover:scale-105 transition-transform hover:bg-white cursor-pointer uppercase"
          >
            <ArrowUpCircle size={18} />
            <span>{t.return_origin}</span>
          </motion.button>
        )}
      </AnimatePresence>
    </section>
