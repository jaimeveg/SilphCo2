...
export default function FullSidebar() {
  ...
  // DETECCIÓN DE IDIOMA (Default: 'es' si falla la detección)
  ...
  const handleNavigation = (id: string) => {
    // Si estamos en la Home de Academy (ej: /es/academy/module-1), scroll suave
    // Si estamos en otro sitio, navegación normal.
    // Simplificación para Fase B:
    if (pathname.includes('/academy/module-1')) {
      ...
      // Navegación forzada al módulo si estamos fuera
      router.push(`/${currentLang}/academy/module-1#${id}`);
    }
  };
            ...
            <div key={category.id} className="relative">
              ...
              >
                ...
                <div className="flex-1 flex items-center justify-between pr-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-200 overflow-hidden whitespace-nowrap">
                  ...
                  <ChevronRight 
                    size={16} 
                    className={`transition-transform duration-300 delay-200 ${isOpen ? 'rotate-90 text-cyan-400' : 'text-slate-700'}`} 
                  />
                </div>
              ...
              <div className={`overflow-hidden transition-all duration-500 delay-200 ease-in-out bg-slate-900/20 ${isOpen ? 'max-h-[80vh] opacity-100' : 'max-h-0 opacity-0'}`}>
                ...
                {/* BRANCH: LINKS (TOOLS) - CON PREFIJO DE IDIOMA */}
