...
export default function LocationMatrix({ locations, lang }: Props) {
  const dict = POKEDEX_DICTIONARY[lang]; // Acceso a la raíz del diccionario (labels, regions, types)
  const [activeGroup, setActiveGroup] = useState<string | null>(null);

  // Agrupar por Versión/Juego (Ej: "Red", "Blue")
  const groupedLocations = useMemo(() => {
    ...
    locations.forEach(loc => {
      // FIX: Accedemos a 'dict.regions' directamente, NO a 'dict.labels.regions'
      // @ts-ignore - Ignoramos error de tipado si la región no existe en el diccionario (fallback a loc.region)
    ...
    if (!activeGroup && keys.length > 0) setActiveGroup(keys[0]);
    
    return groups;
  }, [locations, activeGroup, dict]);
    ...
    <div className="flex flex-col md:flex-row gap-4 h-[350px]">
       ...
       <div className="w-full md:w-2/3 bg-slate-900/60 border border-slate-800 rounded-xl flex flex-col overflow-hidden relative">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar relative z-10">
             {activeGroup && groupedLocations[activeGroup].map((loc, idx) => (
