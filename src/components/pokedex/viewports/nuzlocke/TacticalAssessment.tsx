...
import { Info, Crown, Swords, Shield, Skull, Zap, HelpCircle, Activity } from 'lucide-react';
...
const PhaseCard = ({ label, data, isActive, onClick }: { label: string, data: PhaseData, isActive: boolean, onClick: () => void }) => {
    // CAMBIO VITAL: Ahora es dependiente de !data.available
    const isNA = !data.available;
    return (
        <div 
            onClick={onClick}
            className={cn(
        ...
        >
            <div className="flex justify-between items-center mb-2 border-b border-slate-800/50 pb-2">
                ...
                <span className="text-[9px] font-mono text-slate-500">{data.score}</span>
            </div>
            ...
            <p className={cn("text-[9px] text-center font-medium leading-tight mt-auto pt-2 border-t border-slate-800/50 min-h-[2.5em] flex items-center justify-center transition-colors",
                ...
                {isNA ? "Not obtained in this phase" : data.reason}
            </p>
        </div>
    );
};
...
export default function TacticalAssessment({ analysis }: Props) {
    ...
    useEffect(() => {
        if (!analysis) return;
        // Cambio vital: revisar .available
        if (!analysis.phases.late.available) {
            if (analysis.phases.mid.available) setSelectedPhase('mid');
            else if (analysis.phases.early.available) setSelectedPhase('early');
        } else {
    ...
    const { phases, role, debugLog, phaseLabels } = analysis;
    ...
    const renderStats = () => {
        ...
        if(activeData.specialEvo) {
            items.unshift({ icon: Crown, color: "text-purple-400", text: `This pokemon evolves by a special method (e.g., item, trade, level-up with conditions). Availability may vary.` });
        }
